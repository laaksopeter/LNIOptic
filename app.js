...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxNSyOxtg79177XQSSZtI5w5zEHje0EOQHGag8KDAQXTLlj049xlvCm7UOtPDvRffUl/exec"; // UPDATE THIS

const app = initializeApp(firebaseConfig);
const auth = getAuth(app);
const provider = new GoogleAuthProvider();

let currentPage = 'home', currentStock = [], currentSearch = "", currentTab = "Master";
...
document.getElementById('global-sync').onclick = () => { if(currentPage === 'home') refreshStats(); else loadData(currentTab); };
...
function renderList(items) {
    ...
    const filtered = items.filter(i => (String(i.id)+String(i.size)+String(i.cert)+String(i.loc)+String(i.type)).toLowerCase().includes(query));
    ...
    filtered.forEach(item => {
        ...
        div.innerHTML = `
            <div style="flex:1;">
                ...
                <div style="font-size:0.65rem; color:var(--text-muted);">${isWIP ? 'Cust: '+item.type : 'Grade: '+item.type}</div>
            </div>
            <div style="display:flex; flex-direction:column; gap:5px;">
                <button class="btn-move" onclick="${isWIP?`window.movePart('${item.id}')`:`window.deleteItem('${item.id}','${item.type}')`}" style="padding:6px 10px; border-radius:6px; border:none; cursor:pointer; font-weight:700; background:${isWIP?'#ffedd5':'#fee2e2'}; color:${isWIP?'#f97316':'#dc2626'};">
                    ${isWIP?'MOVE':'USE'}
                </button>
                ${isWIP ? `<button class="btn-primary" onclick="window.completePart('${item.id}')" style="padding:6px 10px; font-size:0.7rem; background:#22c55e;">DONE</button>` : ''}
            </div>
        `;
...
window.completePart = async (id) => {
    if (!confirm(`Mark Part ${id} as Complete?`)) return;
    await fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ action: "COMPLETE", id, user: auth.currentUser.email }) });
    loadData("Master");
};
...
document.getElementById('material-form').onsubmit = async (e) => {
    ...
    btn.innerText = "Enter"; btn.disabled = false;
    loadData(data.item);
};

document.getElementById('bulk-csv-input').onchange = async (e) => {
    const file = e.target.files[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = async (ev) => {
        const rows = ev.target.result.split("\n").filter(r => r.trim() !== "");
        for (let row of rows) {
            const parts = row.split(",").map(i => i.trim());
            let data;
            if (currentPage === 'wip_parts') {
                data = { action: "ADD", page: "wip_parts", item: "Master", id: parts[0], cert: parts[2], size: parts[1], loc: parts[4] || "Laser Cut", type: parts[3], user: auth.currentUser.email };
            } else {
                data = { action: "ADD", page: currentPage, item: matSelect.value, id: parts[0], size: parts[1], thickness: parts[2], cert: parts[3], loc: parts[4], type: matSelect.value, user: auth.currentUser.email };
            }
            await fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify(data) });
        }
        alert("Bulk Import Complete"); loadData(currentPage === 'wip_parts' ? "Master" : matSelect.value);
    };
